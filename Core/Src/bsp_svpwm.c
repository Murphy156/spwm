...
//         *  0 ~ 360°对应 0 ~ 65536，s16degree = 360° / 65536
//         *  使用定点运算，Q = 15格式
//         *  处理精度的理解
//         * */
//        DTheta += round(FREQ_RATIO * 65536); /** round 函数的作用是对一个浮点数进行四舍五入到最接近的整数。 */
//
//        /** 等比例缩小，使得数据范围再10bit以内，其中高两位用于判断象限，低8位用于查表
//         *  对于角度值（°），判断象限可以除以90°，得到的整数部分只有0，1，2，3这四个数值
//         * */
